...
### 6. Make sure README.md links to the image

Check `README.md` for a `## Tracker status` section. If it is missing, add one at the end of the file:
...
If the section already exists, leave `README.md` untouched — the image path is stable, so the existing link keeps working once the file underneath it is overwritten.
